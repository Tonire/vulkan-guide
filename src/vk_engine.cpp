...
#define WITH_RENDERDOC 0
#if WITH_RENDERDOC
#ifdef _WIN32
#define NOMINMAX
#include <windows.h>
#include "renderdoc_app.h"
HMODULE RenderDocMod;
HWND hwnd;
...
{
#if WITH_RENDERDOC
	// Start capture with actual device and window handles
	if (renderdoc_api)
    {
		renderdoc_api->StartFrameCapture(RENDERDOC_DEVICEPOINTER_FROM_VKINSTANCE(_instance), hwnd);
	}
#endif
    ...
    // we will signal the _renderSemaphore, to signal that rendeing has finished
    ...
    // as it's necessary that drawing commands have finihes before the image is displayed to the user
	...
	if (renderdoc_api)
    {
		renderdoc_api->EndFrameCapture(RENDERDOC_DEVICEPOINTER_FROM_VKINSTANCE(_instance), hwnd);
	}
#endif
}
...
void VulkanEngine::draw_geometry(VkCommandBuffer cmd)
{
	...
	scissor.extent = { .width = _drawExtent.width, .height = _drawExtent.height };
...
void VulkanEngine::run()
{
    ...
    while (!bQuit) {
        ...
        if (ImGui::Begin("background"))
        {
            ...
            selected.data.data3 = glm::vec4(selected.data.data3.x, selected.data.data3.y, (float)_frameNumber, selected.data.data3.w);

#if WITH_RENDERDOC
            if(ImGui::Button("RenderDoc - Capture Frame"))
            {
				uint32_t captureCount = renderdoc_api->GetNumCaptures();
				if (captureResult == 1 && captureCount > 0)
				{
					uint32_t lastCaptureID = captureCount - 1;
					bool inRange = renderdoc_api->GetCapture(lastCaptureID, NULL, NULL, NULL);
					if (inRange) // if capture is valid
					{
						if (renderdoc_api->IsTargetControlConnected())
						{
						...
						else
						{
							...
							uint32_t processPID = renderdoc_api->LaunchReplayUI(1, ""); // you can specify arguments for the new renderdoc UI
						}
					}
					else
					{
						fmt::println("Failed to retrieve renderdoc capture.");
					}
				}
            }
#endif
...
AllocatedImage VulkanEngine::create_image(VkExtent3D size, VkFormat format, VkImageUsageFlags usage, bool bMipmapped)
{
...
AllocatedImage VulkanEngine::create_image(void* data, VkExtent3D size, VkFormat format, VkImageUsageFlags usage, bool bMipmapped)
{
    ...
    AllocatedImage newImage = create_image(size, format, usage | VK_IMAGE_USAGE_TRANSFER_DST_BIT | VK_IMAGE_USAGE_TRANSFER_SRC_BIT, bMipmapped);
...
void VulkanEngine::init_vulkan()
{
	if (_instance != VK_NULL_HANDLE) {
		return; // Vulkan instance already created
	}


#if WITH_RENDERDOC
    HMODULE renderDocMod = GetModuleHandle("renderdoc.dll");
    if (!renderDocMod)
    {
        renderDocMod = LoadLibrary("renderdoc.dll");
    }
	if (renderDocMod)
	{
		pRENDERDOC_GetAPI RENDERDOC_GetAPI = (pRENDERDOC_GetAPI)GetProcAddress(renderDocMod, "RENDERDOC_GetAPI");
		int ret = RENDERDOC_GetAPI(eRENDERDOC_API_Version_1_6_0, (void**)&renderdoc_api);
		if (ret == 1 && renderdoc_api) {
			renderdoc_api->SetCaptureOptionU32(eRENDERDOC_Option_DebugOutputMute, 0);
			renderdoc_api->SetCaptureOptionU32(eRENDERDOC_Option_CaptureAllCmdLists, 1);
			renderdoc_api->SetCaptureOptionU32(eRENDERDOC_Option_AllowFullscreen, 1);
			renderdoc_api->SetCaptureOptionU32(eRENDERDOC_Option_AllowVSync, 1);
			//renderdoc_api->SetCaptureOptionU32(eRENDERDOC_Option_APIValidation, 1);
			renderdoc_api->SetCaptureOptionU32(eRENDERDOC_Option_CaptureCallstacks, 1);
			renderdoc_api->SetCaptureOptionU32(eRENDERDOC_Option_RefAllResources, 1);
			renderdoc_api->SetCaptureOptionU32(eRENDERDOC_Option_SaveAllInitials, 1);

			fmt::println("RenderDoc API successfully initialized.");
		}
	}
#endif
	...
	//make vulkan instance with debug features
	builder.set_app_name("Toni app")
...
void VulkanEngine::init_sync_structures()
{
    ...
    // once fence to control when the GPU has finished rendering
    // and 3 semaphores to synchronize rendering with swapchain
...
void VulkanEngine::init_background_pipelines()
{
    ...
    if (!vkutil::load_shader_module("../../shaders/sky.comp.spv", _device, &skyShader) != VK_SUCCESS)
    {
        fmt::println("Error when building the compute shader \n");
    }
    ...
    sky.name = "sky";
    sky.data = {};
    sky.data.data3 = glm::vec4(0.2, -0.06, (float)_frameNumber, 0.0);
...
void VulkanEngine::init_mesh_pipeline()
{
    ...
    push_constant_ranges.size = sizeof(GPUDrawPushConstants);
