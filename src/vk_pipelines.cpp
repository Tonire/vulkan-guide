...
{
    // clear all of the structs back to 0 and with their correct sType
...
void vkutil::PipelineBuilder::set_shaders(VkShaderModule vs, VkShaderModule fs)
{
    _shaderStages.clear();
    _shaderStages.push_back(
        vkinit::pipeline_shader_stage_create_info(VK_SHADER_STAGE_VERTEX_BIT, vs)
    );

    _shaderStages.push_back(
        vkinit::pipeline_shader_stage_create_info(VK_SHADER_STAGE_FRAGMENT_BIT, fs)
    );
}
...
void vkutil::PipelineBuilder::enable_depth_test(bool bDepthWriteEnable, VkCompareOp op)
{
    _depthStencil.depthTestEnable = VK_TRUE;
    _depthStencil.depthWriteEnable = bDepthWriteEnable;
